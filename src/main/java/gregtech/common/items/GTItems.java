...
*/
    private static Item register(String name, Item item) {
        return Registry.register(Registry.ITEM, new Identifier(GTValues.MODID, "metaitems" + "/" + name), item);
    }
...
//    }
    ...
    static {
        ...
        LARGE_FLUID_CELL_STEEL = register("large.fluid.cell.steel", new GTItem(new GTItemSettings().fluidStats(new FluidStats(FluidAmount.ofWhole(64), true)).setMaxStackSize(16)));
        LARGE_FLUID_CELL_TUNGSTEN_STEEL = register("large.fluid.cell.tungstensteel", new GTItem(new GTItemSettings().fluidStats(new FluidStats(FluidAmount.ofWhole(256), true)).setMaxStackSize(16)));
...
*/
        ...
        BATTERY_ULV_TANTALUM = register("battery.ulv.tantalum", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(1000, VoltageTier.ULV))));

        // todo .setModelAmount(8)
        BATTERY_LV_CADMIUM = register("battery.lv.cadmium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(120000L, VoltageTier.LV))));
        BATTERY_LV_LITHIUM = register("battery.lv.lithium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(100000L, VoltageTier.LV))));
        BATTERY_LV_SODIUM = register("battery.lv.sodium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(80000L, VoltageTier.LV))));

        // todo .setModelAmount(8)
        BATTERY_MV_CADMIUM = register("battery.mv.cadmium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(420000L, VoltageTier.MV))));
        BATTERY_MV_LITHIUM = register("battery.mv.lithium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(400000L, VoltageTier.MV))));
        BATTERY_MV_SODIUM = register("battery.mv.sodium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(360000L, VoltageTier.MV))));

        // todo .setModelAmount(8)
        BATTERY_HV_CADMIUM = register("battery.hv.cadmium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(1800000L, VoltageTier.HV))));
        BATTERY_HV_LITHIUM = register("battery.hv.lithium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(1600000L, VoltageTier.HV))));
        BATTERY_HV_SODIUM = register("battery.hv.sodium", new GTItem(new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(1200000L, VoltageTier.HV))));

        // TODO .setModelAmount(8)
        ENERGY_CRYSTAL = register("energy.crystal", new GTItem((GTItemSettings) new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(4000000L, VoltageTier.HV)).maxCount(1)));
        LAPOTRON_CRYSTAL = register("lapotron.crystal", new GTItem((GTItemSettings) new GTItemSettings().electricStats(ElectricStats.createRechargeableBattery(10000000L, VoltageTier.EV)).maxCount(1)));
        ...
        RUBBER_DROP = register("rubber.drop"); // todo .setBurnValue(200);

        FLUID_FILTER = register("fluid.filter");
        ...
        ITEM_FILTER = register("cover.item.filter");
        ORE_DICTIONARY_FILTER = register("cover.ore.dictionary.filter");
        SMART_FILTER = register("cover.smart.item.filter");
        ...
        FLUID_CELL = register("fluid.cell", new GTItem(new GTItemSettings().fluidStats(new FluidStats(FluidAmount.BUCKET, false))));
        INTEGRATED_CIRCUIT = register("circuit.integrated"); // todo .addComponents(new IntCircuitBehaviour());
        FOAM_SPRAYER = register("foam.sprayer"); // todo .addComponents(new FoamSprayerBehavior());

        GELLED_TOLUENE = register("gelled.toluene");

        //IItemContainerItemProvider selfContainerItemProvider = itemStack -> itemStack;
        WOODEN_FORM_EMPTY = register("wooden.form.empty");
        WOODEN_FORM_BRICK = register("wooden.form.brick"); // todo .addComponents(selfContainerItemProvider);
        ...
        PLANT_BALL = register("plant.ball"); // todo .setBurnValue(75);
        ENERGIUM_DUST = register("energium.dust");

        POWER_UNIT_LV = register("power.unit.lv", new GTItem(new GTItemSettings().electricStats(ElectricStats.createElectricItem(100000L, VoltageTier.LV)).setMaxStackSize(8)));
        POWER_UNIT_MV = register("power.unit.mv", new GTItem(new GTItemSettings().electricStats(ElectricStats.createElectricItem(400000L, VoltageTier.MV)).setMaxStackSize(8)));
        POWER_UNIT_HV = register("power.unit.hv", new GTItem(new GTItemSettings().electricStats(ElectricStats.createElectricItem(1600000L, VoltageTier.HV)).setMaxStackSize(8)));
        JACKHAMMER_BASE = register("jackhammer.base", new GTItem(new GTItemSettings().electricStats(ElectricStats.createElectricItem(1600000L, VoltageTier.HV)).setMaxStackSize(4)));
        ENERGY_FIELD_PROJECTOR = register("energy.field.projector", new GTItem(new GTItemSettings().electricStats(ElectricStats.createElectricItem(16000000L, VoltageTier.EV)).setMaxStackSize(1)));

        NANO_SABER = register("nano.saber", new GTItem(new GTItemSettings().electricStats(ElectricStats.createElectricItem(4000000L, VoltageTier.HV)).setMaxStackSize(1))); // todo .addComponents(new NanoSaberBehavior())
        ...
        INGOT_MIXED_METAL = register("ingot.mixed.metal");
        ADVANCED_ALLOY_PLATE = register("plate.advanced.alloy");
        INGOT_IRIDIUM_ALLOY = register("ingot.iridium.alloy");
        PLATE_IRIDIUM_ALLOY = register("plate.iridium.alloy");
        NEUTRON_REFLECTOR = register("neutron.reflector");
        ...
        FIBER_BOARD = register("board.fiber.reinforced");
        MULTILAYER_FIBER_BOARD = register("board.multilayer.fiber.reinforced");
        PHENOLIC_BOARD = register("board.phenolic");
        PLASTIC_BOARD = register("board.plastic");
        WETWARE_BOARD = register("board.wetware");

        VACUUM_TUBE = register("circuit.vacuum.tube"); // todo.setUnificationData(OrePrefix.circuit, Tier.Primitive);
        ...
        SMALL_COIL = register("component.small.coil");
        ...
        ADVANCED_SYSTEM_ON_CHIP_WAFER = register("wafer.advanced.system.on.chip");
        INTEGRATED_LOGIC_CIRCUIT_WAFER = register("wafer.integrated.logic.circuit");
        CENTRAL_PROCESSING_UNIT_WAFER = register("wafer.central.processing.unit");
        HIGH_POWER_INTEGRATED_CIRCUIT_WAFER = register("wafer.high.power.integrated.circuit");
        NAND_MEMORY_CHIP_WAFER = register("wafer.nand.memory.chip");
        NANO_CENTRAL_PROCESSING_UNIT_WAFER = register("wafer.nano.central.processing.unit");
        NOR_MEMORY_CHIP_WAFER = register("wafer.nor.memory.chip");
        POWER_INTEGRATED_CIRCUIT_WAFER = register("wafer.power.integrated.circuit");
        QBIT_CENTRAL_PROCESSING_UNIT_WAFER = register("wafer.qbit.central.processing.unit");
        RANDOM_ACCESS_MEMORY_WAFER = register("wafer.random.access.memory");
        SYSTEM_ON_CHIP_WAFER = register("wafer.system.on.chip");

        CRYSTAL_CENTRAL_PROCESSING_UNIT = register("crystal.central.processing.unit");
        CRYSTAL_SYSTEM_ON_CHIP = register("crystal.system.on.chip");
        ADVANCED_SYSTEM_ON_CHIP = register("plate.advanced.system.on.chip");
        INTEGRATED_LOGIC_CIRCUIT = register("plate.integrated.logic.circuit");
        CENTRAL_PROCESSING_UNIT = register("plate.central.processing.unit");
        HIGH_POWER_INTEGRATED_CIRCUIT = register("plate.high.power.integrated.circuit");
        NAND_MEMORY_CHIP = register("plate.nand.memory.chip");
        NANO_CENTRAL_PROCESSING_UNIT = register("plate.nano.central.processing.unit");
        NOR_MEMORY_CHIP = register("plate.nor.memory.chip");
        POWER_INTEGRATED_CIRCUIT = register("plate.power.integrated.circuit");
        QBIT_CENTRAL_PROCESSING_UNIT = register("plate.qbit.central.processing.unit");
        RANDOM_ACCESS_MEMORY = register("plate.random.access.memory");
        SYSTEM_ON_CHIP = register("plate.system.on.chip");

        BASIC_CIRCUIT_LV = register("circuit.basic"); // todo .setUnificationData(OrePrefix.circuit, Tier.Basic);
        BASIC_ELECTRONIC_CIRCUIT_LV = register("circuit.basic.electronic"); // todo .setUnificationData(OrePrefix.circuit, Tier.Basic);
        ADVANCED_CIRCUIT_PARTS_LV = register("circuit.advanced.parts"); // todo .setUnificationData(OrePrefix.circuit, Tier.Basic);
        ...
        PROCESSOR_ASSEMBLY_HV = register("circuit.processor.assembly"); // todo .setUnificationData(OrePrefix.circuit, Tier.Advanced);
        NANO_PROCESSOR_HV = register("circuit.nano.processor"); // todo .setUnificationData(OrePrefix.circuit, Tier.Advanced);

        NANO_PROCESSOR_ASSEMBLY_EV = register("circuit.nano.processor.assembly"); // todo .setUnificationData(OrePrefix.circuit, Tier.Extreme);
        QUANTUM_PROCESSOR_EV = register("circuit.quantum.processor"); // todo .setUnificationData(OrePrefix.circuit, Tier.Extreme);

        DATA_CONTROL_CIRCUIT_IV = register("circuit.data.control"); // todo .setUnificationData(OrePrefix.circuit, Tier.Elite);
        CRYSTAL_PROCESSOR_IV = register("circuit.crystal.processor"); // todo .setUnificationData(OrePrefix.circuit, Tier.Elite);

        ENERGY_FLOW_CIRCUIT_LUV = register("circuit.energy.flow"); // todo .setUnificationData(OrePrefix.circuit, Tier.Master);
        WETWARE_PROCESSOR_LUV = register("circuit.wetware.processor"); // todo .setUnificationData(OrePrefix.circuit, Tier.Master);

        WETWARE_PROCESSOR_ASSEMBLY_ZPM = register("circuit.wetware.assembly"); // todo .setUnificationData(OrePrefix.circuit, Tier.Ultimate);
        WETWARE_SUPER_COMPUTER_UV = register("circuit.wetware.super.computer"); // todo .setUnificationData(OrePrefix.circuit, Tier.Superconductor);
        WETWARE_MAINFRAME_MAX = register("circuit.wetware.mainframe"); // todo .setUnificationData(OrePrefix.circuit, Tier.Infinite);

        ENGRAVED_CRYSTAL_CHIP = register("engraved.crystal.chip");
        ENGRAVED_LAPOTRON_CHIP = register("engraved.lapotron.chip");

        TURBINE_ROTOR = register("turbine.rotor"); // todo .addComponents(new TurbineRotorBehavior());
